...
- Pass `-d FILE`, configure a saved database, or pass `--thread ID` to skip onboarding.
...
- **Bypass with a file.** In a fresh run, start `saber -d "$FIXTURE"` with a placeholder `OPENAI_API_KEY` and send Ctrl+D after about 4 seconds. It reaches the interactive session (`Welcome to SQLsaber!`, slash commands) without the onboarding setup prompt (`Let's get you set up`, `What would you like to name this connection?`). The fresh default model is `openai:gpt-5.6-sol`.
...
- Onboarding is bypassed by any explicit `-d` selector and by `--thread`.
