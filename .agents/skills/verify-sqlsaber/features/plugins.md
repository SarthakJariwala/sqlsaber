...
- **Show.** Capture `saber plugins show notebook`. Require `Auto-loading: enabled`, `backend` effective `docker` from `default`, and secret fields as `hidden` / `not applicable` when inactive.
- **Non-interactive setup.** Run `"$VERIFY_SQLSABER" drive "$RUN_ID" --evidence plugins/setup-notebook.txt -- uv run saber plugins setup notebook --set backend=docker --yes`. Then `saber plugins setup sandbox --set provider=docker --yes`. Output includes `Saved … settings. Changes apply to new CLI sessions.`
...
- **Disable and enable.** Capture `saber plugins disable viz` (`viz disabled for new CLI sessions.`), list (`viz` `disabled`), then `saber plugins enable viz` and list (`viz` `defaults`).
...
- `viz` has no settings declaration. `show`/`setup`/`set` fail with `exposes no settings`; use enable/disable only.
