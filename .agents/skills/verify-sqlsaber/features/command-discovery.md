...
- `discovery-root-help` lists the `auth`, `db`, `knowledge`, `models`, `plugins`, `theme`, `threads`, and `rpc` commands.
...
- **Root help entry.** Run `.agents/skills/verify-sqlsaber/bin/verify-sqlsaber drive "$RUN_ID" --evidence command-discovery/root-help.txt -- uv run saber --help`. Exit code `0`, the heading contains `Commands`, and all eight command families appear, including `plugins`.
