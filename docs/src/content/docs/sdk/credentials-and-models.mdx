...
`Config.in_memory(subagent_models=...)` accepts only `handoff`. Plugin nested
models are capability configuration, for example
`NotebookConfig(model=pin("openai:gpt-5-mini"))`. See
[Per-tool model overrides](/sdk/advanced/#per-tool-model-overrides).
