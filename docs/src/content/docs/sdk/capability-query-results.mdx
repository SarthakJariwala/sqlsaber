...
`analyze_data`, `viz`, and sandbox Python resolve `result_*.json` handles from conversation history. They load bytes from `PluginContext.query_result_store`. Build one `PluginContext` with the store we already passed to `SqlTools`. Call `discover_capabilities` in this host process. `SQLSaber` does not call it for us.
...
Sandbox stays absent when no sandbox provider is configured. That empty return is not a second store.
...
`SQLSaber` already owns one session store and always attaches `SqlTools` plus `Knowledge`. It does not load notebook, viz, or sandbox unless we pass them. The CLI passes `load_capability_factories()` so installed plugins still appear. Do the same in Python when we want that CLI set.
