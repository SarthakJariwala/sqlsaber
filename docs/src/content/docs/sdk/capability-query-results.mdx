...
`analyze_data`, `viz`, and `analyze_in_sandbox` load complete SQL results from
`PluginContext.query_result_store`. Create a context with the store we passed to
`SqlTools`, then pass that context to the plugins:
...
main = resolve_model(settings.auth, settings.model.name)
...
All four capabilities now read from the same store. For sandbox configuration and
session cleanup, see [Persistent sandbox analysis](/sdk/capabilities/#persistent-sandbox-analysis).
...
The example prints `250`. The plugin factories receive the session's
`PluginContext`, so notebook, viz, and sandbox share the store with `execute_sql`.
