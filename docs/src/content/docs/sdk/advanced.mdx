---
title: Advanced
description: Nested plugin models, custom system prompts, write access, and injectable knowledge and thread managers in the SQLsaber SDK.
---

import { Aside } from "@astrojs/starlight/components";

## Nested plugin models

Pin a nested plugin model on the capability config. That is the same channel the CLI writes through `saber plugins setup`. A pin never reuses the main API key.
    ...
    capabilities=[
        partial(
            ...
            config=NotebookConfig(
                model=pin("openai:gpt-5-mini", api_key="sk-..."),
            ),
        ),
    ],
)
...
Omit `model`, or pass `INHERIT`, to use the main agent model.
