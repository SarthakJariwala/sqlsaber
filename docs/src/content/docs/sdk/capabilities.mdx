...
options = SQLSaberOptions(
    database="analytics",
    capabilities=[*load_capability_factories(), WebSearch()],
)
...
`load_capability_factories()` returns the CLI plugin set. That includes notebook, viz, and sandbox when those packages are installed. Factories run later with the session `PluginContext`, so they share the session `query_result_store`.
...
`PluginContext` supplies the database registry, knowledge manager, dangerous-mode flag, normalized tool model overrides, the required shared `QueryResultStore`, and the optional application-owned artifact store. A factory may return one capability or a sequence of capabilities. Managed `SQLSaber` does not call `discover_capabilities`. Pass `load_capability_factories()` into `SQLSaberOptions.capabilities`, or call `discover_capabilities(context)` yourself on a host agent.
