...
Pass that same store into `PluginContext.query_result_store` before you construct notebook, viz, or sandbox. Each capability that omits the store creates its own empty `InMemoryQueryResultStore`, so analysis tools cannot load `execute_sql` rows.
...
`PluginContext` supplies the database registry, knowledge manager, dangerous-mode flag, normalized tool model overrides, the shared `QueryResultStore`, and the optional application-owned artifact store. A factory may return one capability or a sequence of capabilities.
