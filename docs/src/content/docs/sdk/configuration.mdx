...
| `capabilities` | `Sequence[AbstractCapability[Any] \| Callable]` | `()` | Explicit pydantic-ai capabilities and plugin factories (see [Capabilities](/sdk/capabilities)). SQLSaber always includes SQL and knowledge tools. The CLI passes `load_capability_factories()`. |
...
When `sqlsaber-notebook` is listed in `SQLSaberOptions.capabilities` (the CLI
does this via `load_capability_factories()`), `workspace_input_resolver` lets a
