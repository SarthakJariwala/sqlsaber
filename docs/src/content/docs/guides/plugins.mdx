...
The `saber` CLI loads installed plugins from the `sqlsaber.capabilities` entry-point group. An embedded `SQLSaber` session does not. Pass `SQLSaberOptions(capabilities=load_capability_factories())` when Python should use the same plugins as the CLI.
