...
handoff uses the legacy `saber models set --agent handoff` override, if present,
then the active session model. A plugin model takes precedence over that legacy
setting; unsetting it restores the fallback. Authentication uses the configured
