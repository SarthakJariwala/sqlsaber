---
title: Models
description: Choose the model SQLsaber uses for SQL generation. Set the default, configure plugin models, and configure thinking.
---
...
## Configure a plugin model

Set a model for handoff, viz, or notebook without changing the main model:

```bash
saber plugins set handoff model openai:gpt-5
saber plugins set viz model openai:gpt-5
saber plugins set notebook model openai:gpt-5
saber plugins show handoff model
saber plugins unset handoff model
```

If no plugin model is set, the plugin uses the session model. Changes apply to new
CLI sessions. Legacy `models --agent` commands are no longer supported, and old
subagent overrides are ignored. Set those models again through `saber plugins`.
