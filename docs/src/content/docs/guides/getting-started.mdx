...
You need an API key from Anthropic, OpenAI, Google, Groq, xAI, Mistral, Cohere,
or Hugging Face. You can instead use a ChatGPT subscription through OpenAI
Codex.
...
On first launch, SQLsaber asks you to choose an AI provider. API-key providers
use an environment variable or your operating system credentials store. OpenAI
Codex uses browser authentication with a ChatGPT subscription.
